var init = function () {
	...
	$('#navwatchhistory').addClass('current');
	$('.singleheader').text(localStorage.getItem("watch_history"));
	/*menu active*/
	$("#watchhistory").addClass("navActive");

	function watchHistoryResp(data, textStatus, xhr) {
		console.log(data);
		if (data.code == 200) {
			//$('#spinner').hide();
			if (data.list.length > 0) {
				...
				for (var i = 0; i < data.list.length; i++) {
					var content_types_id = data.list[i].content_types_id;
					if (content_types_id == "1" || content_types_id == "2" || content_types_id == "3" || content_types_id == "4") {
						...
						if (data.list[i].posterForTv != '') {
							favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + data.list[i].content_types_id + '" data-permalink="' + data.list[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="' + data.list[i].posterForTv + '" class="img-fluid" alt=""><p class="see-more"></p></div></div></div>';
						} else {
							// favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + res.list[i].content_types_id + '" data-permalink="' + res.list[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="images/languageimage.jpg" class="img-fluid" alt=""><p class="see-more">' + res.list[i].name + '</p></div></div></div>';
							favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + data.list[i].content_types_id + '" data-permalink="' + data.list[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="images/no_image_tv.png" class="img-fluid" alt=""></div></div></div>';
						}
					}
					if (data.list.length == (i + 1)) {
						$('#spinner').hide();
						$('#watchHistorySecBody').removeClass('hide');
					}
				}
				$('#nocontent').hide();
				if (favoriteData != '') {
					$('.watchHistorySec').empty().append(favoriteData);
				}
			} else {
				$('#spinner').hide();
	...
	var URL = baseurl + "watchHistory?" + DATA;
	makeGetAjaxCall(URL, 30000, watchHistoryResp);
	...
	function checkKey(e) {
		...
		if (remoteBtnCode == '38') {
			...
			if (($("#spinner").is(":visible") == true)) {
				...
				if (currentparentId === "watchHistorySec") {
			...
			if (($("#spinner").is(":visible") == true)) {
				...
				if (currentparentId === "mainmenu") {
					if ($("#watchHistorySec").children().length != 0) {
						$current.removeClass("current");
						$('.watchHistorySec').children().eq(0).addClass("current");
					}
				} else if (currentparentId === "watchHistorySec") {
					var position = parseInt($('#' + currentId).attr('position'));
					var activePosition = position + 4;
					if ($('[position=' + activePosition + ']').length > 0) {
						$current.removeClass("current");
