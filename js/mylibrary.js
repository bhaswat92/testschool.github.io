var init = function () {
	...
	$('#navmylibrary').addClass('current');
	$('.singleheader').text(localStorage.getItem("my_library"));
	/*menu active*/
	$("#mylibrary").addClass("navActive");

	function myLibraryResp(data, textStatus, xhr) {
		console.log(data);
		if (data.code == 200) {
			if (data.mylibrary.length > 0) {
				var favoriteData = '';
				var j = 0;
				var k = 0;
				for (var i = 0; i < data.mylibrary.length; i++) {
					var content_types_id = data.mylibrary[i].content_types_id;
					if (content_types_id == "1" || content_types_id == "2" || content_types_id == "3" || content_types_id == "4") {
						...
						if (data.mylibrary[i].posterForTv != '') {
							favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + data.mylibrary[i].content_types_id + '" data-permalink="' + data.mylibrary[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="' + data.mylibrary[i].posterForTv + '" class="img-fluid" alt=""><p class="see-more"></p></div></div></div>';
						} else {
							// favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + res.mylibrary[i].content_types_id + '" data-permalink="' + res.mylibrary[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="images/languageimage.jpg" class="img-fluid" alt=""><p class="see-more">' + res.mylibrary[i].name + '</p></div></div></div>';
							favoriteData += '<div class="cusWidth cus-langdiv favRow' + k + '" id="favCol' + rand + '_' + k + '" position="' + j + '" content_types_id="' + data.mylibrary[i].content_types_id + '" data-permalink="' + data.mylibrary[i].permalink + '"><div class="cutom-card"> <div class="cutom-cardbody"> <img src="images/no_image_tv.png" class="img-fluid" alt=""></div></div></div>';
						}
					}
					if (data.mylibrary.length == (i + 1)) {
						$('#spinner').hide();
						$('#myLibrarySecBody').removeClass('hide');
					}
				}
				$('#nocontent').hide();
				if (favoriteData != '') {
					$('.myLibrarySec').empty().append(favoriteData);
				}
			} else {
				$('#spinner').hide();
	...
	var URL = baseurl + "MyLibrary?" + DATA;
	makeGetAjaxCall(URL, 30000, myLibraryResp);
	...
	function checkKey(e) {
		...
		if (remoteBtnCode == '38') {
			...
			if (($("#spinner").is(":visible") == true)) {
				...
				if (currentparentId === "myLibrarySec") {
			...
			if (($("#spinner").is(":visible") == true)) {
				...
				if (currentparentId === "mainmenu") {
					if ($("#myLibrarySec").children().length != 0) {
						$current.removeClass("current");
						$('.myLibrarySec').children().eq(0).addClass("current");
					}
				} else if (currentparentId === "myLibrarySec") {
					var position = parseInt($('#' + currentId).attr('position'));
					var activePosition = position + 4;
					if ($('[position=' + activePosition + ']').length > 0) {
						$current.removeClass("current");
